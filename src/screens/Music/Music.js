...
const Music = ({ navigation, route }) => {
    ...
    useEffect(() => {
        axios.get(`${serverConfig.ip}/getPlayList/${route.params.albumName}`).then(res => {
                    ...
                    <SafeAreaView style={{ flex: 1 }}>
                        <ScrollView>
                            ...
                            <View style={{ alignItems: 'center', marginTop: 10 }}>
                                <View>
                                    <Image source={{ uri: `${route.params.albumImage}` }} style={{ width: 250, height: 250, borderRadius: 30 }} />
                                </View>
                                <View style={{ marginTop: 15 }}>
                                    <Text style={{ color: 'white', fontSize: 22, fontWeight: '900' }}>{route.params.albumName}</Text>
                                </View>
                            </View>
